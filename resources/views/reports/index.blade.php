...
@section('content')
    <div class="mb-6 flex flex-col items-stretch gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div class="min-w-0">
            ...
            <h1 class="mt-1 text-2xl font-semibold sm:text-3xl">Mijn rapporten</h1>
        </div>

        <a href="{{ route('invoices.index') }}" class="w-full rounded-md bg-yellow-500 px-4 py-2 text-center font-semibold text-gray-950 hover:bg-yellow-400 sm:w-auto">
            Facturen
        </a>
    </div>
        ...
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="divide-y divide-gray-100 bg-white">
                @forelse($reports as $report)
                    <tr>
                        <td class="max-w-xs break-words px-6 py-4 text-sm font-medium">{{ $report->original_pdf_filename }}</td>
                        <td class="whitespace-nowrap px-6 py-4 text-sm text-gray-600">{{ $report->week_number }}</td>
