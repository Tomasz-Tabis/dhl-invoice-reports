...
@section('content')
    <div class="mb-6 flex items-center justify-between gap-4">
        <div>
            ...
            <h1 class="mt-1 text-3xl font-semibold">Opgeslagen facturen</h1>
        </div>
        ...
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="divide-y divide-gray-100 bg-white">
                @forelse($invoiceUploads as $invoiceUpload)
                    <tr>
                        <td class="px-6 py-4 text-sm font-medium">{{ $invoiceUpload->original_pdf_filename }}</td>
                        <td class="px-6 py-4 text-sm text-gray-600">{{ $invoiceUpload->week_number }}</td>
                        <td class="px-6 py-4 text-sm text-gray-600">{{ $invoiceUpload->year }}</td>
                        <td class="px-6 py-4 text-sm text-gray-600">{{ $invoiceUpload->created_at?->format('Y-m-d H:i') }}</td>
                        <td class="px-6 py-4 text-sm text-gray-600">{{ $invoiceUpload->reports_count }}</td>
